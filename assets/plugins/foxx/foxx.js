...
const fx_tpl = ({
	toast: (type, data, id) => {
		...
		return '<div>\
        <i class="fx-icons-type   ' + data.settings.icons[type] + '"></i>\
        </div>\
        <div class="content">\
        ...
        <div>\
        <i id="' + id + '-end" class="fx_exit fx-icons-exit"></i>\
        </div>\
        '
	}
})
...
const fx_build = ({
	toast: function (type, data) {
		...
		setTimeout(() => {
			...
			document.getElementById(itemID + '-end').addEventListener("click", () => {
				data.settings.timeOut = false;
				fx_build.end.toast(itemID, data.settings)
			});
		}, data.settings.delay);
	},
