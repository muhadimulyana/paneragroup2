const fx_init = ({
	toast: ({
		...
		dismissable: true,
		icons: ({
			success: 'fx-icons-check',
			info: 'fx-icons-info',
			warning: 'fx-icons-block',
			error: 'fx-icons-exit'
		}),
		options: () => {
			return fx_init.toast.position + ' ' + fx_init.toast.css + ' ' + fx_init.toast.animateIn
		}
...
const fx = ({
	toast: ({
		neutral: (data) => {
			fx_build.toast('neutral', data, fx_init.toast.timeOut, fx_init.toast.animateOut)
		},
		success: (data) => {
			fx_build.toast('success', data, fx_init.toast.timeOut, fx_init.toast.animateOut)
		},
		info: (data) => {
			fx_build.toast('info', data, fx_init.toast.timeOut, fx_init.toast.animateOut)
		},
		warning: (data) => {
			fx_build.toast('warning', data, fx_init.toast.timeOut, fx_init.toast.animateOut)
		},
		error: (data) => {
			fx_build.toast('error', data, fx_init.toast.timeOut, fx_init.toast.animateOut)
		},
		init: ({
			icons: (type) => {
				switch (type) {
		...
		template: (type, data) => {
			return '<div>\
            <i class="fx-icons-type   ' + fx.toast.init.icons(type) + '"></i>\
        </div>\
        <div class="content">\
        ...
        <div>\
            <i class="fx_exit fx-icons-exit"></i>\
        </div>\
        '
		}
	}),
